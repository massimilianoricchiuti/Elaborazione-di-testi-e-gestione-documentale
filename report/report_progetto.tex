...
\date{A.A. 2025/2026}
\input{generated/consistenza.tex}
\begin{document}
\maketitle
\vspace{-1.0cm}
...
Le analisi calcolano copertura delle fonti e dei campi, distribuzioni territoriali, sequenze cronologiche, statistiche economiche e frequenze testuali. Gli importi usano \code{Decimal}; le durate negative restano segnalate e sono escluse dalle statistiche di durata. \code{pypdf}, con ripiego su \code{pdfminer.six}, estrae il testo dei PDF. Risultati JSON, riepilogo CSV dei PDF, grafici SVG e frammenti LaTeX sono generati dalla pipeline; la relazione sintetica usa i conteggi, mentre il \emph{Report dei dati} espone le analisi dettagliate.

Jinja2 genera il sito in \code{dist/}. La Home ha sei sezioni numerate, un indice persistente e collegamenti diretti: Introduzione, Consultazione, Documenti, Progetto e metodo, Documentazione e Compilazione. Le altre pagine sono \emph{Archivio}, \emph{Report dei dati} e \emph{Qualità dei dati}. L'Archivio offre ricerca, filtri e ordinamento; ogni CIG dispone di Scheda completa, Struttura XML e Documenti, che riunisce i file locali. L'XML resta scaricabile; le fonti web dichiarano il nesso con il CIG. Gli HTML originari sono scaricati in ZIP; i PDF e gli XML sono disponibili direttamente. Il vecchio \code{progetto.html} rimanda alla Home. CSS, JavaScript, SVG e download sono locali; i dettagli includono microdata \code{schema.org/GovernmentService}. Senza JavaScript restano accessibili contenuti e collegamenti; ricerca, ordinamento e schede a linguetta richiedono JavaScript.

\section*{4. Limiti}
Il campione è didattico, non rappresentativo del sistema italiano dei contratti pubblici. Campi assenti o non omogenei limitano il confronto; le incongruenze non sono corrette arbitrariamente e le differenze fra importi non sono automaticamente interpretate come ribassi. Il riconoscimento dei documenti richiede il CIG nel nome o nel testo: non è implementato OCR per i PDF immagine. Le fonti web possono mutare e un nesso di contesto, lotto, accordo quadro o fase antecedente non equivale a un riscontro sul CIG esatto.
...
I test controllano parsing, DTD, modelli misti, fonti, estrazione, statistiche, viste XML, microdata, pagine e collegamenti interni. Il workflow esistente prepara il dataset, costruisce il sito, esegue i test e pubblica \code{dist/} a ogni push su \code{main}; anche l’avvio manuale è limitato a questo branch. Le modifiche di \code{development} vengono pubblicate dopo il merge in \code{main}. Il workflow copia il PDF versionato senza compilare LaTeX. Le verifiche coprono struttura e coerenza degli output, non certificano l'esattezza amministrativa delle fonti.

\section*{6. Prompt documentati}
Questa sezione incorpora gli otto prompt e le note già registrati nel progetto e aggiunge la sintesi dell'intervento attuale. Le formulazioni storiche sono conservate: \code{DOCS\_DIR} appartiene a una richiesta iniziale; l'implementazione attuale configura la directory tramite \code{ProjectPaths.xml\_dir}. Gli strumenti generativi sono stati usati per supportare modellazione, codice e documentazione; gli output richiedono controlli sui sorgenti e sui risultati. Le note dei prompt 6--8 documentano gli interventi precedenti, senza attribuire loro verifiche nuove.
...
L'intervento aggiorna i template e la navigazione, mantiene le analisi e i documenti del corpus e concentra descrizione e prompt in questa relazione, scaricabile dalla sezione Documentazione. La numerazione e i collegamenti della Home funzionano come ancore della pagina statica.
\end{document}
